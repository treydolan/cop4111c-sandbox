...
const volumeWarning = document.querySelector("#volumeWarning");
const closeVolumeWarningBtn = document.querySelector("#closeVolumeWarning");
...
lfoToggle.addEventListener("change", () => {
  lfoControls.style.display = lfoToggle.checked ? "flex" : "none";
...
// Close the warning module
closeVolumeWarningBtn.addEventListener("click", () => {
  hideVolumeWarning();
});
...
function showVolumeWarning() {
  ...
  volumeWarning.setAttribute("aria-hidden", "false");
}

function hideVolumeWarning() {
  volumeWarning.classList.remove("show");
  volumeWarning.setAttribute("aria-hidden", "true");
}
